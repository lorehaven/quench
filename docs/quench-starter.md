...
- `http::serve(db, init)` — the quench-http equivalent: same `BASE_PATH`/TLS/health/readiness behavior, built on `discover_and_mount` (below) plus the correlation-id/logger middleware, and `serve_app` (below) for the TLS-detecting dispatch. Covers the common case that needs no extra DI providers or middleware.
- `http::serve_app(service_name, app, container)` — the TLS-detecting tail end of `http::serve`, factored out for a service that can't call `serve` directly because it needs its own DI providers or middleware (`quench_auth::http::middleware::Auth`, most often): HTTPS + an HTTP→HTTPS redirect server when `SERVER_CERT_PATH`/`SERVER_KEY_PATH` resolve, plain HTTP on `SERVER_ADDR` otherwise, served via `quench_http::server::{serve_http, serve_https}`. Pair it with `discover_and_mount` (below) and your own `ContainerBuilder`:
  ```rust
...
- `common::health::HealthState` — `live()`, `is_live()`, `is_ready()`, `mark_ready()`, `spawn_ready_when(init)` (spawns `init`, then marks ready once it completes — the one place `mark_ready`/"initialization complete" logging lives, instead of every service hand-rolling its own `tokio::spawn` wrapper around a dependency wait); backs the `/health`, `/health/live`, `/health/ready` routes (actix: mounted by `actix::routers::health::scope()`; quench-http: `#[get]`-discovered from `http::routers::health`).
