//! The actix-web half of the relying-party authorization-code + PKCE flow:
//! builds the redirect to gatehouse's `/authorize`, and exchanges the code
//! that comes back at `/auth/callback` for this service's own token pair.
//! Config, the refresh call, and the PKCE state shape live in
//! `crate::domain::sso_client` (framework-agnostic, shared with
//! `crate::http::domain::sso_client`) - this module is only the part that
//! touches `HttpRequest`/`HttpResponse`.
